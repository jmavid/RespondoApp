...
export function DocumentList() {
  ...
  useEffect(() => {
    const subscription = supabase
      .channel(`documents_${Math.random()}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'documents'
        ...
        // If status changed to completed, refresh signed URLs
        if (payload.new.status === 'completed') {
          getSignedUrl(payload.new.id, payload.new.storage_path);
        }
      })
      .subscribe();
  ...
  const getSignedUrl = async (docId, storagePath) => {
    try {
      ...
      setViewUrls(prev => ({ ...prev, [docId]: signedUrl }));
    } catch (error) {
    ...
    try {
      ...
      for (const doc of data) {
        if (doc.status === 'completed') {
          getSignedUrl(doc.id, doc.storage_path);
        }
      }
    } catch (error) {
          ...
          .then(() => {
            setDocuments(prev => prev.map(doc =>
              doc.id === documentData.id
                ? { ...doc, status: 'error', error_message: error.message }
                : doc
            ));
          });
      });
